#include "nearMissDetection.h"

namespace NearMissDetection
{

  size_t
  NEARMISS::WriteCallback(void *contents, size_t size, size_t nmemb, std::string *userp)
  {
  ...
  int NEARMISS::create_input_sources(gpointer pipe, gpointer mux, guint num_sources)
  {
  ...
  void
  NEARMISS::changeBBoxColor(gpointer obj_meta_data, int has_bg_color, float red, float green,
  ...
  void
  NEARMISS::addDisplayMeta(gpointer batch_meta_data, gpointer frame_meta_data)
  {
  ...
  void
  NEARMISS::setObjColPost(NvDsFrameMeta* frame_meta,
  std::vector<int> is_closed_matrix) {
    int counter = 0;
    int itr = -1;
    for (NvDsMetaList * l_obj = frame_meta->obj_meta_list; l_obj != NULL;
      ...
      // if itr is inside closed matrix set red

      if (std::count(is_closed_matrix.begin(), is_closed_matrix.end(), itr)) {
        changeBBoxColor(obj, 1, 1.0, 0.0, 0.0, 0.25);
      }else {
        changeBBoxColor(obj, 1, 0.0, 1.0, 0.0, 0.25);
      }
  ...
  GstPadProbeReturn
  NEARMISS::tiler_src_pad_buffer_probe(GstPad *pad, GstPadProbeInfo *info,
                                   gpointer u_data)
  {
         ...
         l_frame = l_frame->next)
    {
      ...
      std::vector<std::vector<double>> vehicle_dets;
      std::vector<std::vector<double>> person_dets;
           ...
           l_obj = l_obj->next)
      {
        ...
        if (class_index == PGIE_CLASS_ID_VEHICLE)
        {
          ...
          vehicle_dets.push_back({x1, y1, x2, y2, itr});

          changeBBoxColor(obj_meta, 1, 0.0, 1.0, 0.0, 0.25);
        }  

        if (class_index == PGIE_CLASS_ID_PERSON)
        {
          ...
          person_dets.push_back({x1, y1, x2, y2, itr});

          changeBBoxColor(obj_meta, 1, 0.0, 1.0, 0.0, 0.25);
        }

        if (person_dets.size() > 0 && vehicle_dets.size() > 0) {
          std::vector<int> itr_vals;

          for (int i = 0; i < person_dets.size(); i++)
          {
            for (int j = 0; j < vehicle_dets.size(); j++)
            {
                double x2OfFirst = person_dets[i][2];
                double x1OfSecond = vehicle_dets[j][0];

                double distance = x2OfFirst - x1OfSecond;
                
                if( distance < threshold) {
                  // push itr of vehicle and person
                  itr_vals.push_back(person_dets[i][4]);
                  itr_vals.push_back(vehicle_dets[j][4]);
                } 
            }
          }
          NEARMISS::setObjColPost(frame_meta, itr_vals);
        }
      }
  ...
  gboolean
  NEARMISS::bus_call(GstBus *bus, GstMessage *msg, gpointer data)
  {
  ...
  void
  NEARMISS::cb_newpad(GstElement *decodebin, GstPad *decoder_src_pad, gpointer data)
  {
  ...
  void
  NEARMISS::decodebin_child_added(GstChildProxy *child_proxy, GObject *object, gchar *name, gpointer user_data)
  {
  ...
  GstElement *
  NEARMISS::create_source_bin(guint index, gchar *uri)
  {
  ...
  gchar *
  NEARMISS::get_absolute_file_path(gchar *cfg_file_path, gchar *file_path)
  {
  ...
  gboolean
  NEARMISS::set_tracker_properties(GstElement *nvtracker)
  {
    gboolean ret = FALSE;
    GError *error = NULL;
    gchar **keys = NULL;
  ...
  int NEARMISS::configure_element_properties(int num_sources, GstElement *streammux, GstElement *pgie_vehicle_detector,
                                         GstElement *nvtracker, GstElement *sink, GstElement *tiler)
  {
    ...
    if (boost::filesystem::exists(boost::filesystem::path(
            NearMissDetection::NEARMISS::PGIE_VEHICLE_ENGINE_PATH)))
    {

      g_object_set(G_OBJECT(pgie_vehicle_detector),
                   "model-engine-file", NearMissDetection::NEARMISS::PGIE_VEHICLE_ENGINE_PATH.c_str(), NULL);
    }
    else
    {
    ...
    if (!NearMissDetection::NEARMISS::set_tracker_properties(nvtracker))
    {
  ...
  void NEARMISS::setPaths(guint num_sources)
  {
    // Config Paths
...
{
  NearMissDetection::NEARMISS nearmiss;
  GMainLoop *loop = NULL;
  ...
  ctx = g_option_context_new("NEARMISS DeepStream App");
  group = g_option_group_new("NEARMISS", NULL, NULL, NULL, NULL);
  g_option_group_add_entries(group, nearmiss.entries);
  ...
  gint sources = nearmiss.create_input_sources(pipeline, streammux, num_sources);
  if (sources == -1)
  {
    return -1;
  }
  ...
  int fail_safe = nearmiss.configure_element_properties(num_sources, streammux, pgie_vehicle_detector,
  ...
  bus_watch_id = gst_bus_add_watch(bus, nearmiss.bus_call, loop);
  gst_object_unref(bus);

/* Set up the pipeline */
#ifdef PLATFORM_TEGRA
  if (!nearmiss.display_off)
  {
  ...
  else
  {
    gst_pad_add_probe(tiler_src_pad, GST_PAD_PROBE_TYPE_BUFFER,
                      nearmiss.tiler_src_pad_buffer_probe, NULL, NULL);
  }
